...
WORD Cpu::absoluteAddressing() {
  regPC += 2;
  return ((WORD)(read(regPC - 1) << 8)) | read(regPC - 2);
}
...
WORD Cpu::indirectAddressing() {
  ...
  if (read(regPC - 2) == 0xFF)
    return (read(regPC - 2) | read(regPC - 1) << 8) + 1;
    //return read(((WORD)read(regPC - 1) << 8) | read(regPC - 2)) 
    // | ((WORD)read(((((WORD)read(regPC - 1) << 8) | read(regPC - 2)))) << 8);
  else
    return read(((WORD)read(regPC - 1) << 8) | read(regPC - 2))
    | ((WORD)read(((((WORD)read(regPC - 1) << 8) | read(regPC - 2)) + 1)) << 8);
}
...
void Cpu::BRK(addressingMode mode) {
  ...
  regPC = (read(0xFFFE) | ((WORD)read(0xFFFF) << 8));
  cycles = 7;
}
